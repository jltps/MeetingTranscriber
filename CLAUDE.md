...
1. **Update the docs first.** Add the new version to `README.md` (status
   line + a new `vX.Y.Z` section + the document map) and to
   `SHIPPED_HISTORY.md` (append the new VXYZ paragraph at the bottom of the
   list — that file holds the per-roadmap changelog that used to live in
   `CLAUDE.md`'s "Already shipped beyond v1" section). If the version
   corresponds to a roadmap folder (e.g. `roadmap/VXXX/`), commit any
   untracked roadmap files in that folder and mark its
   `ROADMAP_00_INDEX.md` as shipped.
...
- **Already shipped beyond v1 — see [`SHIPPED_HISTORY.md`](SHIPPED_HISTORY.md).**
  That file owns the per-roadmap changelog (V04 / V05 / V06 / V062 / V07 / V0.7.1
  / V072 / V073 / V074 / V075 / V076) — what was built and why. Don't rebuild
  any of it; extend the existing code. Use the SHIPPED_HISTORY entry to find
  the relevant `roadmap/VXYZ/` folder for the full per-block plan.
